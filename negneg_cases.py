...
def group_vars_by_cip(interpreted_genomes_json):
    ...
    Returns:
        Dictionary of variants grouped by CIP (key = CIP, value = list of variant objects)
    """
    ...
    for ig in interpreted_genomes_json:
        ...
        if ig_obj.variants:
            vars_by_cip[ig_obj.interpretationService.lower()] = ig_obj.variants
    return vars_by_cip
...
def group_vars_by_tier(variants_json):
    ...
    for variant in variants_json:
        ...
        if top_rank_tier == 4:
            ...
            tiered_vars['TIER{tier}'.format(tier=top_rank_tier)].append(variant)
    return tiered_vars    
...
def is_neg_neg(ir_json, cips=[]):
    ...
    gel_tiered_vars = group_vars_by_tier(vars_by_cip['genomics_england_tiering'])
    ...
    cip_candidates = 0
    for cip in cips:
        cip_candidates += len(vars_by_cip[cip]) if (cip in vars_by_cip) else 0
...
def group_cases():
    ...
    for case in guys_cases:
        ...
        if count < 3:
            ...
            # Some very old pulot cases have broken formatting causing error here, so catch these with try/except
            try:
